...
router.post("/upload", protect, upload.single("file"), processExcelFile, async (req, res) => {
  try {
    ...
    await logActivity(req.user.id, "upload_file", `Uploaded file: ${fileRecord.originalName}`)
    await createNotification(req.user.id, "success", `File ${fileRecord.originalName} was uploaded and processed successfully.`)
...
router.delete("/:id", protect, async (req, res) => {
  try {
    ...
    await logActivity(req.user.id, "delete_file", `Deleted file: ${file.originalName}`)
